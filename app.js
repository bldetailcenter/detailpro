...
async function subirFoto(event, intervencionId, columna) {
  ...
  const fileName = `${intervencionId}_${columna}_${Date.now()}.${fileExt}`;
  ...
  // 3. Guardar URL en la tabla de intervenciones
  const updateData = {};
  updateData[columna] = publicUrl;
  ...
  if (updateError) {
    showToast('Error al guardar el enlace de la foto', 'error');
    return;
  }

  showToast('✓ Imagen guardada');
  verIntervencion(intervencionId); // Recargar la ventana
}

async function eliminarFoto(intervencionId, tipo) {
  if (!confirm('¿Seguro que quieres eliminar esta foto?')) return;
  
  const columna = tipo === 'antes' ? 'foto_antes' : 'foto_despues';
  const updateData = {};
  updateData[columna] = null;

  await db.from('intervenciones').update(updateData).eq('id', intervencionId);
  verIntervencion(intervencionId); // Recargar la ventana
}
