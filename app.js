// ═══════════════════════════════════════════════════════════
//  DetailPro — app.js (VERSIÓN FINAL UNIFICADA)
//  Todo operativo: Fotos, Edición, Borrado, PDF y Almacén
// ═══════════════════════════════════════════════════════════
...
async function handleLogin() {
  ...
  if (error) { showToast('Error de acceso', 'error'); return; }
...
function verProducto(id) {
  ...
  document.getElementById('modal-prod-contenido').innerHTML = `
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;">
      <div class="card">Stock: ${fmt(p.stock_actual,0)} ml</div>
      <div class="card">Precio/Dosis: ${fmt(p.precio_por_dosis,2)}€</div>
    </div><br>
    <p>${p.observaciones || 'Sin notas'}</p>
  `;
  abrirModal('modal-producto');
}

// ═══════════════════════════════════════════════════════════
//  OPERACIONES (EL "MOTOR" DE LA WEB)
// ═══════════════════════════════════════════════════════════
function addProductoUsado() {
  ...
  div.innerHTML = `
    <select class="pu-producto"><option value="">Seleccionar...</option>${options}</select>
    <input type="number" class="pu-ml" placeholder="ml" />
...
async function crearIntervencion() {
  ...
  if(!mat || !cli) { showToast('Faltan datos', 'error'); return; }
...
async function verIntervencion(id) {
  ...
  document.getElementById('modal-int-contenido').innerHTML = `
    <div style="display:grid; gap:1rem">
      ${estadoBadge(i.estado)}
      <p>Cliente: ${i.cliente_nombre}</p>
      <div style="display:grid; grid-template-columns:1fr 1fr; gap:0.5rem">
        <div class="card" style="text-align:center">
          <small>ANTES</small><br>
          ${i.foto_antes ? `<img src="${i.foto_antes}" style="width:100%"/>` : `<input type="file" onchange="subirFoto(event,'${i.id}','foto_antes')"/>`}
        </div>
        <div class="card" style="text-align:center">
          <small>DESPUÉS</small><br>
          ${i.foto_despues ? `<img src="${i.foto_despues}" style="width:100%"/>` : `<input type="file" onchange="subirFoto(event,'${i.id}','foto_despues')"/>`}
        </div>
      </div>
      <div style="display:flex; gap:0.5rem">
        ...
        <button class="btn-secondary" onclick="eliminarIntervencion('${i.id}')" style="color:#ef4444">🗑️ Borrar</button>
      </div>
    </div>`;
...
async function subirFoto(e, id, col) {
  ...
  const { error } = await db.storage.from('fotos_vehiculos').upload(name, file, { contentType: file.type });
  if (error) { showToast('Error subida', 'error'); return; }
...
async function generarPDFCliente() {
  ...
  if(i.foto_antes || i.foto_despues) {
    ...
    if(i.foto_antes) {
      const img = await getDataUrl(i.foto_antes);
      if(img) doc.addImage(img, 'JPEG', 15, 30, 180, 100);
    }
    if(i.foto_despues) {
      const img = await getDataUrl(i.foto_despues);
      if(img) doc.addImage(img, 'JPEG', 15, 140, 180, 100);
    }
  }
...
function addDamagePoint(e) {
  ...
  document.getElementById('damage-points').appendChild(c);
}

function resetMapaDanos() {
  damagePoints = []; damageCounter = 0;
  const gp = document.getElementById('damage-points');
  if(gp) gp.innerHTML = '';
}
